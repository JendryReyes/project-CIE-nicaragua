...
import { ArrowLeft, Calculator, CheckCircle2, AlertTriangle, XCircle, Upload, FileCheck2, Clock, ExternalLink, PauseCircle, Send, Lock, FileDown } from "lucide-react";
...
import { keyDe, leerQuincena, guardarQuincena, cambiarEstado, pasosCierre, type EstadoQuincena } from "@/lib/estado-quincena";
...
function CierreQuincena() {
  ...
  const totales = useMemo(() => {
    ...
    const montoDescontado = resumenes.reduce((s, r) => s + r.montoSuspendido, 0);
  ...
  const calcular = async () => {
    ...
    setEstado(nuevo);
    guardarQuincena({ key: qkey, estado: nuevo, totalFacturable: 0, totalNinos: 0 });
  ...
  const cerrar = () => {
    ...
    setEstado("cerrada");
    guardarQuincena({ key: qkey, estado: "cerrada", cerradaEn: new Date().toISOString(), totalFacturable: totales.monto, totalNinos: totales.total });
    ...
    <div className="space-y-6 max-w-[1400px]">
              ...
              <div key={p.id} className="flex-1 flex items-center">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`h-8 w-8 rounded-full grid place-items-center text-xs font-medium shrink-0 ${
                    done ? "bg-[oklch(0.55_0.13_155)] text-white" :
                    active ? "bg-foreground text-background" :
                    "bg-muted text-muted-foreground"
                  }`}>{done ? "✓" : i + 1}</div>
                  <div className="min-w-0">
                    <div className={`text-sm font-medium truncate ${active ? "" : "text-muted-foreground"}`}>{p.label}</div>
                    <div className="text-[10px] text-muted-foreground truncate">{p.desc}</div>
                  </div>
                </div>
                {i < pasosCierre.length - 1 && (
                  <div className={`flex-1 h-px mx-3 ${done ? "bg-[oklch(0.55_0.13_155)]" : "bg-border"}`} />
                )}
              </div>
            );
          })}
      ...
      <div className="rounded-2xl border border-border/70 bg-card p-3 flex flex-wrap gap-3 items-center text-sm">
        ...
        <div className="ml-auto flex gap-2">
          {!calculado && (
            <button onClick={calcular} disabled={procesando}
          ...
          {calculado && estado === "lista" && (
            <button onClick={cerrar} disabled={totales.bloq > 0}
        ...
        <>
          ...
          {totales.montoDescontado > 0 && (
            <div className="rounded-xl border border-border/70 bg-card px-4 py-2.5 text-sm text-muted-foreground flex items-center gap-2">
              ...
              Se descontaron <b className="text-foreground">${totales.montoDescontado.toFixed(2)}</b> por suspensiones activas
            </div>
          )}
            ...
            {resumenes.map((r) => (
              <button
                ...
                className={`w-full text-left grid grid-cols-[1fr_110px_85px_85px_85px_85px_115px_120px] gap-3 px-4 py-3 border-b border-border/40 text-sm hover:bg-muted/30 transition-colors ${
                  ...
                  : r.estado === "suspendido" ? "bg-[oklch(0.97_0.04_55)]"
                  : ""
                }`}
              >
                ...
                <span className={`tabular text-right self-center ${r.horasSuspendidas > 0 ? "text-[oklch(0.55_0.15_55)] font-medium" : "text-muted-foreground"}`}>
...
function KPI({ label, value, tone }: { label: string; value: string; tone?: "ok" | "warn" | "bad" | "info" }) {
  const cls =
    ...
    tone === "info" ? "border-[oklch(0.7_0.13_55/0.4)] bg-[oklch(0.97_0.04_55)]" :
...
function EstadoBadge({ estado }: { estado: "ok" | "revisar" | "bloqueado" | "suspendido" }) {
  ...
  if (estado === "suspendido") return <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider rounded-full px-2 py-0.5 bg-[oklch(0.94_0.07_55)] text-[oklch(0.45_0.13_55)]"><PauseCircle className="h-3 w-3" /> Pausa</span>;
...
function ConciliacionDrawer({ resumen, onClose, onConstancia, readonly }: { resumen: ResumenFacturacionNino; onClose: () => void; onConstancia: (id: string) => void; readonly: boolean; }) {
  return (
    <>
          ...
          {resumen.suspension && (
            <div className="rounded-xl bg-[oklch(0.97_0.04_55)] border border-[oklch(0.7_0.13_55/0.4)] p-4">
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                {rangoSuspensionLegible(resumen.suspension)} · {resumen.horasSuspendidas}h descontadas (${resumen.montoSuspendido.toFixed(2)})
              </p>
            </div>
          )}
